...
class COCODataset(JointsDataset):
    ...
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        self.detail_eval = cfg.TEST.DETAIL_EVAL
        self.is_train = is_train
        self.max_patch = cfg.DATASET.MAX_PATCH
        self.patch_mode = cfg.DATASET.PATCH_MODE
        ...
        self.use_cocomini = cfg.DATASET.USE_COCOMINI
        ...
        # logger.info('=> db_images: {}'.format(len(self.db)))
        logger.info('=> db_patches: {}'.format(self.count))
    ...
    def _load_coco_keypoint_annotations(self):
        ...
        for index in self.image_set_index:
            ...
            if self.use_cocomini:
                if res != []:
                    if self.image_set == 'train2017':
                        if len(res[0]['annos']) > 1:
                            gt_db.extend(res)
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        if rec == []:
            ...
            if self.patch_mode == 'window' and len(rec) > self.max_patch:         
                res = []
                split_rec = [rec[i:i+self.max_patch] for i in range(0, len(rec), self.max_patch)]
                for split_item in split_rec:
                    res.append({
                        'image': self.image_path_from_index(index),
                        'filename': '',
                        'annos': split_item
                    })
                    self.count += len(split_item)
            else:
                res = [{
                    'image': self.image_path_from_index(index),
                    'filename': '',
