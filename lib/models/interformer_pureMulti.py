...
class TransPoseH(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.position_embedding = build_position_encoding(self.trans_size, d_model, mode=multi_pos_embedding, vec_dim=d_model)
    ...
    def forward(self, x, pos_mask, length):
        # x [sum(length), 3, 256, 192]
        device = x.device
        x = self.deal_by_backbone(x)                    # [sum(length), 96, 64, 48]
        pos_mask = self.padding_tensor(pos_mask, length, Learnable=False)
        pos_mask = self.position_embedding(pos_mask)    # [sum(length), 96, 16, 12]

        bs = len(length)
        _, c, h, w = x.shape
        mask = self.get_mask(length, (h,w))
        mask = mask.to(device)
        x = self.padding_tensor(x, length)              # [bs, n, c, h, w]

        src, multi_pos, mask = self.flatten_input(x, pos_mask, mask)
        
        if self.use_multi_pos == False:
            multi_pos = None

        x = self.global_encoder(src, src_key_padding_mask=mask, pos=multi_pos)    # [L, bs, c]
        x = x.permute(1, 2, 0).contiguous().view(bs, c, max(length), h, w)
        x = x.permute(0, 2, 1, 3, 4).contiguous().view(bs*max(length), c, h, w)
